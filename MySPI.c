...
//#define	LCD_RS_SET  	GPIO_SetBits(GPIOB,GPIO_Pin_0)  //PB0 决定数据还是命令，0命令1数据
//#define	LCD_BLK_SET  	GPIO_SetBits(GPIOB,GPIO_Pin_1)  //PB1背光引脚 
...
void Lcd_SetXY(u16 x,u16 y)
{
	  ...
	  Lcd_SetRegion(x,y,x+1,y+2);
	#else		
	  Lcd_SetRegion(x,y,x+2,y+1);
	#endif
}
...
void Gui_DrawPoint(u16 x,u16 y,u16 Data)
{
  ...
  Lcd_SetRegion(x,y,x+2,y+3);
	
	#else	
	
	Lcd_SetRegion(x,y,x+(128-x),y+2); //竖屏显示区域设置为该行全行和顺序后两行
...
/*****************************************
 ...
 出口参数：color  点颜色值                                 
******************************************/
uint16_t Lcd_ReadPoint_16(u16 x,u16 y ,uint16_t Data)
{
  uint16_t Data_Return = 0;
  Lcd_SetXY(x,y);
	LCD_CS_CLR;
  LCD_RS_SET;
	Data_Return |= ( MySPI_SwapByte(Data>>8)<<8);	//写入高8位数据
	Data_Return |= MySPI_SwapByte(Data);			//写入低8位数据
  LCD_CS_SET; 
...
{	
   ...
   //Lcd_SetRegion(2,1,X_MAX_PIXEL+1,Y_MAX_PIXEL);
	 Lcd_SetRegion(0,0,X_MAX_PIXEL,Y_MAX_PIXEL+1);
...
#else	
   Lcd_SetRegion(0,0,X_MAX_PIXEL+1,Y_MAX_PIXEL);
